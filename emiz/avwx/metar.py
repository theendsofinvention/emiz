...
# stdlib
# module
from . import core, remarks, service
from .static import FLIGHT_RULES, IN_UNITS, NA_UNITS
from .structs import MetarData, Units
...
def fetch(station: str) -> str:
    ...
    """
    return service.get_service(station)('metar').fetch(station)  # type: ignore
...
def parse_na(txt: str) -> (MetarData, Units):  # type: ignore
    ...
    return MetarData(**wxresp), units  # type: ignore


def parse_in(txt: str) -> (MetarData, Units):  # type: ignore
    """
    ...
    if 'CAVOK' in wxdata:
        wxresp['visibility'] = core.make_number('CAVOK')  # type: ignore
        wxresp['clouds'] = []  # type: ignore
        wxdata.remove('CAVOK')
    else:
        wxdata, wxresp['visibility'] = core.get_visibility(wxdata, units)
    wxresp['other'], wxresp['temperature'], wxresp['dewpoint'] = core.get_temp_and_dew(wxdata)
