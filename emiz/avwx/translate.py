...
from . import core, remarks
from .static import CLOUD_TRANSLATIONS, ICING_CONDITIONS, TURBULANCE_CONDITIONS, WX_TRANSLATIONS
from .structs import (
    Cloud, MetarData, MetarTrans, Number, ReportData,
    TafData, TafLineTrans, TafTrans, Units,
)


def get_cardinal_direction(direction: int) -> str:  # noqa
    """
...
def other_list(wxcodes: [str]) -> str:  # type: ignore
    """
...
def shared(wxdata: ReportData, units: Units) -> typing.Dict[str, str]:
    """
...
def metar(wxdata: MetarData, units: Units) -> MetarTrans:
    ...
    return MetarTrans(**translations)  # type: ignore
...
def taf(wxdata: TafData, units: Units) -> TafTrans:
    ...
    return TafTrans(**translations)  # type: ignore
